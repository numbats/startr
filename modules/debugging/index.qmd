...
When you see the error message ["Error: object '\_\_\_\_\_' not found"]{style="color: red;"}, it usually means there is a typo or the object wasn't loaded in correctly. In this instance, we havent used the correct object. Let's look at how we can fix this.
...
When you read that file in, make sure you avoid absolute paths to it. Not everyone has the same file structure as you, so they won't be able to access your dataset if it is stored at `/home/learnr/Documents/My Job/project_2040/data/example.csv`. Instead provide a short relative oath, such as `example.csv` or `data/example.csv`. If you're uploading files, it's also nice to provide a zip that contains a R script of your minimal reproducible code, along with the data in the folders referenced by the script.
...
the [`reprex`](https://reprex.tidyverse.org/) package is a useful tool that can help you make a reproducible example. It can be used to run the code in your example in a clean environment, which is a good way to test if you've included everything necessary for others to reproduce the error. It'll also copy a neatly formatted version of your example, which includes images and console output from the example as it runs on your computer. It can even include a collapsible summary of your session information.

-   add images
-   potentially a video here
...
-   [ ] Is the problem [clearly]{.underline} and [succinctly described]{.underline}?
-   [ ] Is the [expected solution or behaviour]{.underline} outlined?
-   [ ] Has [nobody asked this question]{.underline} before? (Search the web!)
-   [ ] Are you asking the [right people]{.underline} at the [right place]{.underline}?
-   [ ] Have you added [appropriate tags]{.underline} or [keywords]{.underline} to reach the right experts?
-   [ ] Have you prepared your example using the [reprex package]{.underline} to ensure the problem is easily reproduced on other computers?
-   [ ] Have you [minimised your problem]{.underline} as much as possible, [removing irrelevant packages, data and code]{.underline}?

**If your questions involve a dataset**

-   [ ] Is the data supplied
-   [ ] If the data is big, could you cull your data further to communicate or reproduce the problem?

# Asking for help checklist ☑️

This is **not an exhaustive check-list.** What needs to be included and checked is very question specific, but this checklist should apply to most questions.

-   [ ] Is the problem [**clearly**]{style="background-color: yellow;"} and [**succinctly described**]{style="background-color: yellow;"}?
-   [ ] Is the [**expected solution or behaviour**]{style="background-color: yellow;"} outlined?
-   [ ] Has [**nobody asked this question**]{style="background-color: yellow;"} before? (Search the web!)
-   [ ] Are you asking the [**right people**]{style="background-color: yellow;"} at the [**right place**]{style="background-color: yellow;"}?
-   [ ] Have you added [**appropriate tags**]{style="background-color: yellow;"} or [**keywords**]{style="background-color: yellow;"} to reach the right experts?
-   [ ] Have you prepared your example using the [**reprex package**]{style="background-color: yellow;"} to ensure the problem is easily reproduced on other computers?
-   [ ] Have you [**minimised your problem**]{style="background-color: yellow;"} as much as possible, [**removing irrelevant packages, data and code**]{style="background-color: yellow;"}?

**If your questions involve a dataset**

-   [ ] Is the data supplied
-   [ ] If the data is big, [**could you cull your data further**]{style="background-color: yellow;"} to communicate or reproduce the problem?
