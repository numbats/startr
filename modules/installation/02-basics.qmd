...
<!-- interface picture with 3 panes -->
...
<!-- interface picture with 4 panes, add source pane --> 
...
<!-- put the book in the shelf picture -->
...
<!-- packages window picture and pop-up picture -->

Now that you have the book (package) on the bookshelf, to bring it out to your current working environment, type `library(package_name)` in the **Console** pane. All of the package's functions, data sets, and help files are now available for you to use until you close your current R session. So, if you have closed it, you will need to library the R packages again.

You should library the packages you used at the top of the R Script. This way, you do not need to remember which packages you need in the script. It is also a good practice to do.
...
c(
    ...
    "Did you use the right function for library the package?" = !search_ast(.code, .fn = library),
    "Did you install the right package?" = !search_ast(.code, .expr = install.packages("palmerpenguins")),
    "Did you library the right package?" = !search_ast(.code, .expr = library(palmerpenguins))
)
