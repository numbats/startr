...
Quarto provides a seamless framework for integrating code, text, and visualisations into a single, interactive, and reproducible document. Written in `Markdown`, Quarto is easy to learn yet powerful enough for complex document creation. Built on the R Markdown ecosystem, it supports embedding R code chunks, making reports easy to share, update, and compatible with many existing `Rmd` files with minimal modifications.
