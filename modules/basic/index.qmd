...
The `%%` operator returns the remainder when the number on the left is divided by the number on the right. For example, `7 %% 3`
...
Now, when you evaluate the value of the variable `my_result` by running it, you will
get the value of 8.
...
::: {.callout-tip}
## Hint
Let's say you would like to **assign the result of summing** from
...
c(
  ...
  "Have you spelt the logical value correctly? Note that logical values should be all capital letter (i.e. `TRUE` not `true`)." = brolly_needed,
