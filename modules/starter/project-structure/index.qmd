...
Good project organisation is not just a courtesy; it is essential for collaboration, transparency, and reproducibility in any data analysis project.

This module introduces practical strategies to organise your analysis files. It includes creating and using R Projects, understanding relative and absolute file paths, and structuring your work for clarity and reproducibility. A well organised project sets a strong foundation for reliable data analysis, ultimately saving you time and reducing the risk of errors.
...
Using an R Project streamlines your work by:

1. **Automatic working directory**: RStudio automatically sets the working directory to the project's folder. The working directory is the folder relative file paths are built from (more details [later](#sec-relative)).
2. **Project settings**: RStudio projects restore project-specific settings and keyboard shortcuts (e.g., git, packages, or documents).
3. **Workspace restoration**: Reopen projects with scripts, unsaved changes, and history exactly as you left them.
4. **Organised structure**: Simplifies collaboration and reproducibility with clear folders and relative paths.
...
While you should always save your work, it is best for reproducibility to not save your workspace image. Saving the workspace means storing all of the objects you've created for the next time you open the project. It's better to explicitly load data and scripts each time you open a project, ensuring that the analysis can be replicated by anyone, including your future self.
...
## Expand For Folder Descriptions
...
library(qlcheckr)
apply_checks(
  "Use the `getwd()` function." = search_ast(parse(text = .evaluate_result$.user_code), .fn = getwd),
