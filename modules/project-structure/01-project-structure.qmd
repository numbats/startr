...
In @sec-path, we can see that MacOS and Windows have completely different path structures.
:::
...
It is the location of a file or directory specified relative to your current working directory (the folder you are currently working on).
:::

To understand a bit more about the current working directory, let me introduce the `here` package. It allows you to find your base folder or root directory for you to start on. The `here()` function dynamically identifies the root directory based on the user systems. It allows using a relative path where we only need to specify where in your project folder you are trying to point to.
...
In @sec-abs-path, we talk about the different types of paths and that we should always use a **relative path**. An R project will allow RStudio to set the working directory correctly, handling all of the relative paths for us. It also encourages you to organise the file structure properly by putting all the files used by that project inside the same folder.
...
You should be able to see the project file with .Rproj extension inside the project directory. You have now successfully created the R project. This file extension will allow us to use it as a shortcut for opening the project directly.

:::{.callout-important}
# Self contained

Projects should be self-contained, meaning all files and data should exist inside the project directory.
:::
